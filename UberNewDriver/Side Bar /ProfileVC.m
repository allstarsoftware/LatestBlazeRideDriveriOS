...
{
    [self.btnMenu setTitle:NSLocalizedStringFromTable(@"PROFILEE",[prefl objectForKey:@"TranslationDocumentName"],nil) forState:UIControlStateNormal];
}
            ...
            [afn getDataFromPath:FILE_UPDATE_PROFILE withParamDataImage:dictparam andImage:imgUpload withBlock:^(id response, NSError *error)
             {
                 ...
                 if (response)
                 {
                     ...
                     {
                         UIAlertView *alert=[[UIAlertView alloc]initWithTitle:@"Profile Update Fail" message:[response valueForKey:@"error"] delegate:self cancelButtonTitle:NSLocalizedStringFromTable(@"OK",[prefl objectForKey:@"TranslationDocumentName"],nil) otherButtonTitles:nil, nil];
                         [alert show];
                     }
